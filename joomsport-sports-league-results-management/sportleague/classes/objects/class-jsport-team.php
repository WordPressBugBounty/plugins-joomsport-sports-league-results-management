            ...
            for ($intC = 0; $intC < $count_players; ++$intC) {
                ...
                if($row->player_id){
                    ...
                    for ($intB = 0; $intB < count($ef); ++$intB) {
                        ...
                        $players_object->fields['ef_'.$ef[$intB]->id] = classJsportExtrafields::getExtraFieldValue($ef[$intB], $row->player_id, 0, $this->season_id);
                        ...
                        if(isset($ef_type) && $ef_type == 3){
                            ...
                            $players_object->fields['ef0_'.$ef[$intB]->id] = $orderValue;
                    
                        }
                        if($groupBySelect && $playerPhotoTab){
                            if($ef[$intB]->id == $groupBySelect){
                                if($players_object->fields['ef_'.$ef[$intB]->id]){
                                    $uGroup = $players_object->fields['ef_'.$ef[$intB]->id];
                                }
                            }
                        }
                    }
                    ...
                    if(isset($efPlayerNumber) && isset($efPlayerNumber->id)){
                        $players_object->fields['ef_'.$efPlayerNumber->id] = classJsportExtrafields::getExtraFieldValue($efPlayerNumber, $row->player_id, 0, $this->season_id);
                        ...
                        if(isset($jersey[$this->id])){
                            $players_object->fields['ef_def'] = $jersey[$this->id];
                        }
                    }
                    if(isset($efplayerCard) && isset($efplayerCard->id)){
                        $players_object->fields['ef_'.$efplayerCard->id] = classJsportExtrafields::getExtraFieldValue($efplayerCard, $row->player_id, 0, $this->season_id);
                    }
                    if(isset($efplayerEfSorter) && isset($efplayerEfSorter->id)){
	                    $players_object->fields['ef_'.$efplayerEfSorter->id] = classJsportExtrafields::getExtraFieldValue($efplayerEfSorter, $row->player_id, 0, $this->season_id);
                    }
