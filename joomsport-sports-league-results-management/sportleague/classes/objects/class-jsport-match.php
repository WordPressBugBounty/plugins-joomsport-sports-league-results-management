...
class classJsportMatch
{
    ...
    {
        $m_descr = get_post_meta($this->id,'_joomsport_match_descr',true);
        return classJsportText::getFormatedText($m_descr);
    }
    ...
    public function getTeamEvenetObj(&$events)
    {
        ...
        if ($events) {
            foreach ($events as $key=>$value) {
                ...
                if(JoomsportSettings::get('partdisplay_awayfirst',0) == 1){
                    $tmpEvent->away_value = isset($value['mevents1'])?$value['mevents1']:null;
                    $tmpEvent->home_value = isset($value['mevents2'])?$value['mevents2']:null;
                }else{
                    $tmpEvent->home_value = isset($value['mevents1'])?$value['mevents1']:null;
                    $tmpEvent->away_value = isset($value['mevents2'])?$value['mevents2']:null;
                }
    ...
    public function getLocation($linkable = true)
    {
        if(JoomsportSettings::get('unbl_venue',1)){
            ...
            if ($m_venue) {
                $venue = new classJsportVenue($m_venue, null, false);
