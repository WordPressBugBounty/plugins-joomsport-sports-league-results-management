...
class jsHelperBtw
{
    ...
    public static function getPositions($season_id,$team1,$team2){
        ...
        $obj->maxPosition =  $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->joomsport_season_table} WHERE season_id = %s",
                array($season_id)
            )
        );
        return $obj;
    ...
    public static function getPosition($season_id,$team_id){
        ...
        $res = $wpdb->get_var(
            $wpdb->prepare(
            "SELECT ordering FROM {$wpdb->joomsport_season_table} WHERE season_id = %d AND participant_id = %d"
    ...
    public static function getTeamStat($team_id, $season_id, $matches){
        global $wpdb;
        $matches = array_map("absint",$matches);
        $events = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT me.e_id, SUM(me.ecount) as cnt "
                ." FROM {$wpdb->joomsport_match_events} as me"
                ." WHERE season_id=%d AND t_id=%d"
    ...
    public static function getColumnsOptions($season_id, $team_id){
        ...
        $options["winhome_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
        ...
        $options["drawhome_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
                . " FROM {$wpdb->joomsport_matches} as m"
                . " JOIN {$wpdb->posts} as p ON p.ID = m.postID"
        ...
        $options["losthome_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
                . " FROM {$wpdb->joomsport_matches} as m"
                . " JOIN {$wpdb->posts} as p ON p.ID = m.postID"
        ...
        $options["winaway_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
                . " FROM {$wpdb->joomsport_matches} as m"
                . " JOIN {$wpdb->posts} as p ON p.ID = m.postID"
        ...
        $options["drawaway_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
                . " FROM {$wpdb->joomsport_matches} as m"
                . " JOIN {$wpdb->posts} as p ON p.ID = m.postID"
        ...
        $options["lostaway_chk"] = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*)"
                . " FROM {$wpdb->joomsport_matches} as m"
                . " JOIN {$wpdb->posts} as p ON p.ID = m.postID"
    ...
    public static function showPositionBlock($match_id){
        ...
        $groups = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->joomsport_groups} WHERE s_id = %d",
                array(intval($metas['season_id']))
            )
