...
class jsHelperMatchStatus
{
    ...
    public static function getInstance()
    {
        if (!isset(static::$instance)) {
            global $jsDatabase;
            $query = 'SELECT *'
                .' FROM '.$jsDatabase->db->joomsport_match_statuses.' ORDER BY id';
...
class jsHelperAllTeamPosts
{
    ...
    public static function getInstance()
    {
        ...
        if (!isset(static::$instance)) {
            global $wpdb;
            $teams = $wpdb->get_results("SELECT ID,post_status FROM {$wpdb->posts} WHERE post_type='joomsport_team' AND post_status = 'publish'",OBJECT_K);
...
class jsHelperAllPlayersPosts
{
    ...
    public static function getInstance()
    {
        ...
        if (!isset(static::$instance)) {
            global $wpdb;
            $teams = $wpdb->get_results("SELECT ID,post_status FROM {$wpdb->posts} WHERE post_type='joomsport_player' AND post_status = 'publish'",OBJECT_K);
            /*$argsSeasons = array(
                'posts_per_page'   => -1,
                'offset'           => 0,
...
class jsHelperSeasonPartic
{
    ...
    public static function getInstance($season_id, $participiants, $t_single)
    {
        if(!isset(static::$instance[$season_id])){
            global $wpdb;
            $query = "SELECT ID,post_status FROM {$wpdb->posts} WHERE post_type='".($t_single?'joomsport_player':'joomsport_team')."' AND post_status = 'publish'";
            if($participiants && count($participiants)){
                $query .= " AND ID IN (".implode(",",array_map("absint",$participiants)).")";
            }
            $query .= " ORDER BY post_name";
            $partics = $wpdb->get_results($query);

            static::$instance[$season_id] = $partics;
                /*get_posts(array(
...
class jsHelperTermMatchday
{
    ...
    public static function getInstance()
    {
        if (!isset(static::$instance)) {
            ...
            $tx = JoomsportTermsMeta::getTerms('joomsport_matchday', array("hide_empty" => false, 'orderby' => "id"));
            $mdays = array();
            for($intA=0;$intA<count($tx);$intA++){
                //$term_meta = get_option( "taxonomy_".$tx[$intA]->term_id."_metas");
                $term_meta = JoomsportTermsMeta::getTermMeta($tx[$intA]->term_id);
                $tmp = new stdClass();
...
class jsHelperTermTourn
{
    ...
    public static function getInstance()
    {
        if (!isset(static::$instance)) {
            ...
            $tx = JoomsportTermsMeta::getTerms('joomsport_tournament', array("hide_empty" => false));
