...
class classJsportgetplayers
{
    public static function getPlayersFromTeam($options, $player_id = null)
    {
        ...
        if(isset($group_id) && $group_id){
            $group = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->joomsport_groups} WHERE id=%d ORDER BY ordering",
                    array($group_id)
                )
            );
        ...
        if ((isset($season_id) && $season_id)) {
            ...
            if ($single == 1) {
               ...
               $query = "SELECT l.* ".$sql_select." FROM ".DB_TBL_PLAYER_LIST." as l"
                       .' JOIN '.$wpdb->prefix.'posts as p ON';
                   if(isset($season_id) && count($season_array)){
                       $query .= '  l.season_id IN ('.implode(',', $season_array).') ';
                   }else if(isset($season_id) && $season_id){
                       $query .= '  l.season_id = '.$season_id;

                   }
                $query .= ' AND p.ID = l.player_id AND p.post_status = "publish"'
                        . " WHERE 1=1 "
                ...
                $query_count = "SELECT (l.id) FROM ".DB_TBL_PLAYER_LIST." as l"
                       .' JOIN '.$wpdb->prefix.'posts as p ON';
                if(isset($season_id) && count($season_array)){
                    $query_count .= '  l.season_id IN ('.implode(',', $season_array).') ';
                }else if(isset($season_id) && $season_id){
                    $query_count .= '  l.season_id = '.$season_id;

                }
                $query_count .= ' AND p.ID = l.player_id AND p.post_status = "publish"'
    ...
    public static function getPlayersEvents($season_id = 0)
    {
        ...
        $events = jsHelperEvents::getSeasonEvents($season_id);

        $pickedEvents = array();
        $events_array = array();
        for ($intA = 0; $intA < count($events); ++$intA) {

            $objEvent = new classJsportEvent($events[$intA]);
            $events_array['eventid_'.$events[$intA]] = $objEvent;
            $pickedEvents[] = $events[$intA];
        }
