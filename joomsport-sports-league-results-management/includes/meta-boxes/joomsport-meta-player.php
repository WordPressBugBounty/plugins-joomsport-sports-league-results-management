...
class JoomSportMetaPlayer {
            ...
            <div class="jsBEsettings" style="padding:0px;">
                ...
                <ul class="tab-box">
                    ...
                    if(JoomSportUserRights::isAdmin()){
                        echo wp_kses_post($etabs->newTab(__('Season related settings','joomsport-sports-league-results-management'), 'col_conf', ''));
                    }
            ...
            <div id="col_conf_div" class="tabdiv visuallyhidden">
                <div style="margin-bottom: 25px;margin-left:10px;">
                    ...
                    if(!empty($results)){
                        ...
                        echo '<div style="color:red;">'.esc_html__('Participant is not assigned to any season. Open Main tab and use Assign to season field.', 'joomsport-sports-league-results-management').'</div>';
                    }
    ...
    public static function js_meta_personal($post){
        ...
        <div class="jsminwdhtd jstable">
            ...
            <div class="jstable-row">
                <div class="jstable-cell">
                    <?php echo esc_html__('Short name', 'joomsport-sports-league-results-management');?>
                </div>
                <div class="jstable-cell">
            ...
            <div class="jstable-row">
                <div class="jstable-cell">
                    <?php echo esc_html__('Name in home country', 'joomsport-sports-league-results-management');?>
                </div>
                <div class="jstable-cell">
    ...
    public static function js_meta_teams($post){
        ?>
            <div id="js_player_teamsDIV">
                <?php echo esc_html__('No season selected','joomsport-sports-league-results-management');?>
            </div>
        <?php
    }
    ...
    public static function joomsport_player_save_metabox($post_id, $post){
        ...
        $nonce_name   = isset( $_POST['joomsport_player_nonce'] ) ? sanitize_text_field(wp_unslash($_POST['joomsport_player_nonce'])) : '';
    ...
    private static function saveMetaJersey($post_id){
        ...
        if($season_id){
            $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->joomsport_teamplayers} WHERE seasonID = %d AND playerID = %d",$season_id,$post_id));
            ...
            if($meta_array && count($meta_array) && $season_id){
                foreach($meta_array as $key=>$val){
                    if(isset($_POST["departed"][$key])){
                        $wpdb->query($wpdb->prepare("UPDATE {$wpdb->joomsport_playerlist} SET departed='1' WHERE season_id=%d AND team_id=%d AND player_id=%d",$season_id,$key,$post_id));
                        $wpdb->query($wpdb->prepare("INSERT IGNORE INTO {$wpdb->joomsport_teamplayers}(id,seasonID,teamID,playerID,departed) VALUES(0,%d,%d,%d,'1')",$season_id,$key,$post_id));
                    }else{
                        $wpdb->query($wpdb->prepare("UPDATE {$wpdb->joomsport_playerlist} SET departed='0' WHERE season_id=%d AND team_id=%d AND player_id=%d",$season_id,$key,$post_id));
                        $wpdb->query($wpdb->prepare("INSERT IGNORE INTO {$wpdb->joomsport_teamplayers}(id,seasonID,teamID,playerID,departed) VALUES(0,%d,%d,%d,'0')",$season_id,$key,$post_id));
                    }
                }
            }
        }else{
    ...
    private static function saveMetaEFAssigned($post_id){
        ...
        $meta_array = array();
        if($season_id){
            $efsV = isset($_POST['efs'])?array_map("sanitize_text_field",wp_unslash($_POST['efs'])):array();
            if($efsV && count($efsV)){
                foreach ($efsV as $key => $value){
                    if(isset($_POST['efs_'.$key])){
                        $meta_array[$key] =sanitize_text_field(wp_unslash($_POST['efs_'.$key]));
                    }else{
                        $meta_array[$key] = sanitize_text_field($value);
                    }
                }
