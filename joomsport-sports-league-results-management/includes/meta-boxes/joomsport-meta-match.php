...
class JoomSportMetaMatch {
    public static function output( $post ) {
        ...
        <div class="jsBEsettings" style="padding:0px;">
            ...
            <ul class="tab-box">
                ...
                echo wp_kses_post($etabs->newTab(__('Lineups','joomsport-sports-league-results-management'), 'col_conf', ''));
    ...
    public static function js_meta_descr($post){

        $metadata = get_post_meta($post->ID,'_joomsport_match_descr',true);
         (wp_editor($metadata, 'descr',array("textarea_rows"=>3)));
    ...
    public static function js_meta_playerevents($post){
        ...
        $events = $wpdb->get_results(
                $wpdb->prepare(
                "SELECT id, e_name as name FROM {$wpdb->joomsport_events} WHERE player_event='1' AND dependson='' AND events_sum = '0' AND sportID=%d ORDER BY ordering"
                ,array($sportID)
                )
        );
        ...
        $query = "SELECT me.*,ev.e_name,(subev.e_id) as subevID, GROUP_CONCAT(CONCAT(me.t_id,'*',subev.player_id)) as subevPl, GROUP_CONCAT(subev.player_id) as plFM"
                    ." FROM  {$wpdb->joomsport_match_events} as me"
                . " JOIN {$wpdb->joomsport_events} as ev ON me.e_id = ev.id AND me.match_id = %d"
            .($joomsport_refactoring_v?" LEFT JOIN {$wpdb->joomsport_match_events_addit} as subev ON subev.parent_event = me.id":" LEFT JOIN {$wpdb->joomsport_match_events} as subev ON subev.additional_to = me.id")
    ...
    public static function js_meta_mevents($post){
        ...
        if(!$t_single){
            ...
            $events = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, e_name as name FROM {$wpdb->joomsport_events} WHERE player_event='0' AND sportID=%d ORDER BY ordering"
                    ,array($sportID)
                )
            );
            if(count($events)){
                ...
                $link = get_admin_url(get_current_blog_id(), 'admin.php?page=joomsport-page-events');
                 printf( esc_html__( 'There are no match events. Create new one on %s Extra fields list %s', 'joomsport-sports-league-results-management' ), '<a href="'.esc_url($link).'">','</a>' );
            }
        }
    ...
    public static function js_meta_boxscore($post){
        ...
        if(!count($complexBox)){
            printf(esc_html__('No box score records available. Add new on %sBox score stats list%s','joomsport-sports-league-results-management'),'<a href="admin.php?page=joomsport-page-boxfields">','</a>');
            return;
        }
        ...
        for($intA=0;$intA<count($complexBox); $intA++){
            ...
            if($complexBox[$intA]->complex == '1'){
                $childBox = $wpdb->get_results(
                        $wpdb->prepare(
                        'SELECT * FROM '.$wpdb->joomsport_box.' WHERE parent_id=%d AND published="1" AND ftype="0" ORDER BY ordering,name',
        ...
        if($efbox){
            $season_relation = $wpdb->get_var(
                    $wpdb->prepare(
                    'SELECT season_related FROM '.$wpdb->joomsport_ef.' WHERE id=%d',
                            array($efbox)
                    )
            ) ;
                                ...
                                <tbody>';
                                    ...
                                    for($intPP=0;$intPP<count($players);$intPP++){
                                        ...
                                        $player_stat = $wpdb->get_row(
                                                ...
                                                ,array($post->ID,$home_team,$player->ID))
                                            );
                                ...
                                <tbody>';
                                    ...
                                    for($intPP=0;$intPP<count($players);$intPP++){
                                        ...
                                        $player_stat = $wpdb->get_row(
                                                $wpdb->prepare(
                                                "SELECT * FROM {$wpdb->joomsport_box_match} WHERE match_id=%d AND team_id=%d AND player_id=%d"
                                                ,array($post->ID,$away_team,$player->ID))
                                        );
    ...
    public static function js_meta_general($post){
        ...
        <div>
            ...
            <div class="form-group row">
                <label class="col-sm-2 col-form-label"><?php echo esc_html__('Match duration','joomsport-sports-league-results-management');?></label>
                <div class="col-sm-5">
    ...
    public static function joomsport_match_save_metabox($post_id, $post){
        ...
        $nonce_name   = isset( $_POST['joomsport_match_nonce'] ) ? sanitize_text_field(wp_unslash($_POST['joomsport_match_nonce'])) : '';
    ...
    private static function saveMetaDescr($post_id){

        $meta_data = isset($_POST['descr'])?  wp_kses_post(wp_unslash($_POST['descr'])):'';
        if($meta_data){
            update_post_meta($post_id, '_joomsport_match_descr', $meta_data);
        }else{
            delete_post_meta($post_id, '_joomsport_match_descr');
        }
                ...
                if(isset($post['sub_eventid_vals'][$intA]) && $post['sub_eventid_vals'][$intA]){
                    ...
                    if(count($plisSubArr)){
                        foreach($plisSubArr as $plisSub){
                            ...
                            $newID = jsHelperEvents::addLinkedEvents($eventArr);
        ...
        for($intA=0;$intA<count($sq1);$intA++){
            ...
            $allreadyused = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        ...
                        ." AND s.is_subs!='0'"
                        .' AND s.player_id=%d',
                        array($post_id,$home_team,$sq1[$intA])
                    )
            ...
            $allreadyused = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        ...
                        ." AND s.is_subs!='0'"
                        .' AND s.player_id=%d',
                        array($post_id,$home_team,intval($post['players_team1_in_arr'][$intA]))
                    )
            );
            if(!$allreadyused){
                $wpdb->update($wpdb->joomsport_squad, 
        ...
        for($intA=0;$intA<count($sq2);$intA++){
            ...
            $allreadyused = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        ...
                        ." AND s.is_subs!='0'"
                        .' AND s.player_id=%d',
                        array($post_id,$away_team,intval($sq2[$intA]))
                    )
            );
            if(!$allreadyused){    
                $wpdb->update($wpdb->joomsport_squad, 
            ...
            $allreadyused = (int) $wpdb->get_var(
                    $wpdb->prepare(
                        ...
                        ." AND s.is_subs!='0'"
                        .' AND s.player_id=%d',
                        array($post_id,$away_team,intval($post['players_team2_in_arr'][$intA]))
                    )
            );
            if(!$allreadyused){
                $wpdb->update($wpdb->joomsport_squad, 
    ...
    private static function saveMetaBoxScore($post_id){
       ...
       for($intA=0;$intA<count($h_players);$intA++){
           ...
           $dobl = $wpdb->get_var(
                       $wpdb->prepare(
                       "SELECT id FROM {$wpdb->joomsport_box_match} WHERE match_id=%d AND player_id=%d AND team_id=%d"
           ...
           if($dobl){
               ...
               $wpdb->query(
                   $wpdb->prepare(
                           "INSERT INTO {$wpdb->joomsport_box_match}(match_id,season_id,team_id,player_id".$insert_field.")"
                       . " VALUES(%d,%d,%d,%d".$insert_vals.")",
                           array($post_id,$season_id,$home_team,$h_players[$intA])
                   )
               );
           }
       }
       ...
       for($intA=0;$intA<count($a_players);$intA++){
           ...
           $dobl = $wpdb->get_var(
                   $wpdb->prepare(
                   "SELECT id FROM {$wpdb->joomsport_box_match} WHERE match_id=%d AND player_id=%d AND team_id=%d"
                   ,array($post_id,$a_players[$intA],$away_team)
                   )
           );
           if($dobl){
               ...
               $wpdb->query(
                   $wpdb->prepare(
                       "INSERT INTO {$wpdb->joomsport_box_match}(match_id,season_id,team_id,player_id".$insert_field.")"
                       . " VALUES(%d,%d,%d,%d".$insert_vals.")",
                       array($post_id,$season_id,$away_team,$a_players[$intA])
                   )
               );
           }
       }
