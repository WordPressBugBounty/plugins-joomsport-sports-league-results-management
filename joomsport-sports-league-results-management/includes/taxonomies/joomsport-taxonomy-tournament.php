...
class JoomSportTaxonomyTournament {
    ...
    public static function register_taxonomy(){
        $slug = get_option( 'joomsportslug_joomsport_tournament', null );
        $labels = array(
...
class JoomSportTaxonomyDrop{
    ...
    public function drop_meta_box($post) {
        ...
        ?>
        <div class="tagsdiv" id="<?php echo esc_attr($this->name_slug); ?>">
            <div class="jaxtag">
            <?php 
            ...
            echo '<input type="hidden" name="joomsport_tournament" value="'.esc_attr($current_tournament).'" />';
            //echo '<select>'
     ...
     public  function taxonomy_save_postdata( $post_id ) {
      ...
      if(!isset($_POST[$this->name_slug.'_noncename']))
          return;    
      if ( !wp_verify_nonce( sanitize_text_field(wp_unslash($_POST[$this->name_slug.'_noncename'])), plugin_basename( __FILE__ ) ) )
          return;
