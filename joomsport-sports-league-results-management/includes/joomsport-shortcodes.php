...
class JoomsportShortcodes {
    ...
    public static function joomsport_standings($attr){
        ...
        wp_enqueue_script('jsjoomsport-standings',plugins_url('../sportleague/assets/js/joomsport_standings.js', __FILE__), array('jquery'));
    ...
    public static function joomsport_mday($attr){
        ...
        echo '<input type="hidden" name="jnonce" value="'.(wp_create_nonce("joomsportajaxnonce")).'">';
        return ob_get_clean();
    }
                ...
                if(count($eventsCol)){
                    foreach ($eventsCol as $keyCol=>$valCol){
                        ...
                        $row->{$keyCol} = $wpdb->get_var(
                            ...
                            .' AND pl.player_id = %d'
                            .' AND pl.season_id = %d'
                            ,array($keyCol,DB_TBL_PLAYER_LIST,$row->ID,$args['season_id'])));
                    }
                }
