                ...
                foreach ($matchesC->posts as $row) {
                    ...
                    if ($mdID) {
                        ...
                        if (isset($metas["_joomsport_match_general"][0])) {
                            $metadata = unserialize($metas["_joomsport_match_general"][0]);
                        ...
                        $wpdb->query(
                            $wpdb->prepare(
                                "INSERT IGNORE INTO {$wpdb->joomsport_matches}(postID,mdID,seasonID,teamHomeID,teamAwayID,groupID,status,date,time,scoreHome,scoreAway,duration)"
                                ." VALUES(%d, %d, %d, %d, %d, %d, %d, %s, %s, %f, %f, %d)"
                                ." ON DUPLICATE KEY UPDATE mdID = %d",
                                array($row->ID, $mdID, $season_id, $homeID, $awayID, $group_id, $status, $match_date, $match_time, $score1, $score2, $duration, $mdID)
                            )
                        );
                        if ($wpdb->last_error !== '') :
                            $wpdb->print_error();
