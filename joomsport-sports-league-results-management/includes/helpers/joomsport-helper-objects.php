...
class JoomSportHelperObjects{
    ...
    public static function getTournamentType($season_id){
        ...
        if($term_list && count($term_list)){
            $term_meta = JoomsportTermsMeta::getTermMeta($term_list[0]->term_id,'t_single');
            return $term_meta;
    ...
    public static function getParticipiantSeasons($participiant_id){
        ...
        $posts_array = $wpdb->get_results(
                $wpdb->prepare(
                    'SELECT p.*  FROM '.$wpdb->joomsport_season_table.' as j'
    ...
    public static function getPlayerSeasons($participiant_id){
        ...
        $posts_array = $wpdb->get_results(
                $wpdb->prepare(
                    'SELECT p.*  FROM '.$wpdb->joomsport_playerlist.' as j'
                    . ' JOIN '.$wpdb->posts.' as p ON p.ID = j.season_id '
                    .' WHERE p.post_status = "publish"'
                    .' AND j.player_id = %d'
    ...
    public static function getGroupEdit( $group_id, $post_id ) {
        ...
        ?>
        <div>
            <label><?php echo esc_html__('Group name','joomsport-sports-league-results-management');?></label>
        ...
        <br />
        <div>
            <label><?php echo esc_html__('Use previous season match results','joomsport-sports-league-results-management');?></label>
            <br />
            <?php
            $is_field = array();
    ...
    public static function getSeasonsByTourn($term_id){
        $seasons = get_posts(
    ...
    public static function getSportType($season_id){
        $term_list = get_the_terms($season_id, 'joomsport_tournament');

        if($term_list && count($term_list)){
            $term_meta = JoomsportTermsMeta::getTermMeta($term_list[0]->term_id,'sports_id');
    ...
    public static function getSeasonsBySport($sportID){
        global $wpdb;
        return $seasons = $wpdb->get_col(
                $wpdb->prepare(
            "SELECT tags_rel.object_id
                FROM
                    ...
                    JOIN {$wpdb->termmeta} as tm ON (tm.term_id = tags_tax.term_id AND tm.meta_key='sports_id' AND tm.meta_value=%d)
