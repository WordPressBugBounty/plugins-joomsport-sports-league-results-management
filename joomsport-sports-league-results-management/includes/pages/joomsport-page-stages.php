...
class JoomSportStages_List_Table extends WP_List_Table {
    ...
    public static function record_count() {
        ...
        return $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->joomsport_maps}" );
    }
    public function no_items() {
        echo esc_html__( 'No game stages available.', 'joomsport-sports-league-results-management' );
    }
    function column_name( $item ) {
        ...
        $actions = array(
          'delete' => sprintf( '<a href="?page=%s&action=%s&gamestage=%s&_wpnonce=%s">Delete</a>', (isset($_REQUEST['page'])?( sanitize_text_field(wp_unslash($_REQUEST['page'])) ):''), 'delete', absint( $item['id'] ), $delete_nonce )
        );
    ...
    public function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
          ...
          if ( ! wp_verify_nonce( $nonce, 'joomsport_delete_gamestage' ) ) {
            ...
            self::delete_stage( isset($_GET['gamestage'])?absint( $_GET['gamestage'] ):0 );
...
class JoomSportStages_Plugin {
	...
	public function plugin_settings_page() {
		?>
		<div class="wrap">
			<h2><?php echo esc_html__('Game stages', 'joomsport-sports-league-results-management');?>
                        <a class="add-new-h2"
                                 href="<?php echo esc_url(get_admin_url(get_current_blog_id(), 'admin.php?page=joomsport-gamestages-form'));?>"><?php echo esc_html__('Add new', 'joomsport-sports-league-results-management')?></a>
                        </h2>
	...
	public function screen_option() {
        ...
        if($mscr){
            update_user_meta(get_current_user_id(), 'stages_per_page', $mscr);
    ...
    public static function view(){
        ...
        if (isset($_REQUEST['nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'])), basename(__FILE__))) {
            ...
            if (isset($_REQUEST['id'])) {
                $item = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->joomsport_maps} WHERE id = %d", intval($_REQUEST['id'])), ARRAY_A);
                if (!$item) {
                    $item = $default;
        ...
        <div class="wrap">
            ...
            <h2><?php echo esc_html__('Game stage', 'joomsport-sports-league-results-management')?> <a class="add-new-h2"
                                        href="<?php echo esc_url(get_admin_url(get_current_blog_id(), 'admin.php?page=joomsport-page-gamestages'));?>"><?php echo esc_html__('back to list', 'joomsport-sports-league-results-management')?></a>
            </h2>
    ...
    <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
        ...
        <tr class="form-field">
            <th valign="top" scope="row">
                <label for="name"><?php echo esc_html__('Name', 'joomsport-sports-league-results-management')?></label>
            </th>
            <td>
        ...
        <tr>
            <td width="200" valign="middle">
                <?php echo esc_html__('Separate player stats in match', 'joomsport-sports-league-results-management'); ?>
            </td>
            <td>
        ...
        <tr>
            <td width="200" valign="middle" class="blockHSAutomatic">
                <?php echo esc_html__('Divide by minutes', 'joomsport-sports-league-results-management'); ?>
            </td>
            <td class="blockHSAutomatic">
                <?php echo esc_html__('from', 'joomsport-sports-league-results-management'); ?>
