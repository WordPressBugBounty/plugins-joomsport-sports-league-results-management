...
class JoomSportBoxFields_List_Table extends WP_List_Table {
    ...
    public static function record_count() {
        ...
        return $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->joomsport_box}" );
    }
    public function no_items() {
        echo esc_html__( 'No box fields available.', 'joomsport-sports-league-results-management' );
    }
    function column_name( $item ) {
        ...
        $actions = array(
          'delete' => sprintf( '<a href="?page=%s&action=%s&boxfield=%s&_wpnonce=%s">Delete</a>', (isset($_REQUEST['page'])?( sanitize_text_field(wp_unslash($_REQUEST['page'])) ):''), 'delete', absint( $item['id'] ), $delete_nonce )
        );
    ...
    public function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
          ...
          if ( ! wp_verify_nonce( $nonce, 'joomsport_delete_boxfield' ) ) {
            ...
            self::delete_boxfield( isset($_GET['boxfield'])?absint( $_GET['boxfield'] ):0 );
...
class JoomSportBoxField_Plugin {
	...
	public function plugin_settings_page() {
		...
		?>
		<div class="wrap">
			<h2><?php echo esc_html__('Box score stats', 'joomsport-sports-league-results-management');?>
                        <a class="add-new-h2"
                                 href="<?php echo esc_url(get_admin_url(get_current_blog_id(), 'admin.php?page=joomsport-boxfields-form'));?>"><?php echo esc_html__('Add new', 'joomsport-sports-league-results-management')?></a>
                        </h2>
    ...
    public static function view(){
        ...
        if (isset($_REQUEST['nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'])), basename(__FILE__))) {
            ...
            if(isset($_POST['options']) && count($_POST['options'])){
                $item['options'] = (array_map( 'sanitize_text_field', wp_unslash( $_POST['options'] ) ));
            }
            if(isset($_POST['options']['extraVals'])) {
                $item['options']['extraVals'] = array_map('sanitize_text_field', wp_unslash($_POST['options']['extraVals']));
            ...
            if ($item_valid === true) {
                $item['options'] = wp_json_encode($item['options']);
                if ($item['id'] == 0) {
            ...
            if (isset($_REQUEST['id'])) {
                $item = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->joomsport_box} WHERE id = %d", intval($_REQUEST['id'])), ARRAY_A);
                if (!$item) {
                    $item = $default;
        ...
        <div class="wrap">
            ...
            <h2><?php echo esc_html__('Box score record', 'joomsport-sports-league-results-management')?> <a class="add-new-h2"
                                        href="<?php echo esc_url(get_admin_url(get_current_blog_id(), 'admin.php?page=joomsport-page-boxfields'));?>"><?php echo esc_html__('back to list', 'joomsport-sports-league-results-management')?></a>
            </h2>
...
<div >
    ...
    <div class="jsBepanel">
		...
		<table>
			<tr>
				<td width="250">
                                    <?php echo esc_html__('Record name', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
			...
			<tr>
				<td width="250">
                                    <?php echo esc_html__('Composite', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
                                    <?php ?>
			...
			<tr  class="jshideforcomposite">
				<td width="250">
                                    <?php echo esc_html__('Parent', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
                                    <?php ?>
                                    <?php  echo 'Available in <a href="http://joomsport.com/web-shop/joomsport-for-wordpress.html?utm_source=js-st-wp&utm_medium=backend-wp&utm_campaign=buy-js-pro">Pro Edition</a> only';  ?>
			...
			<tr  class="jshideforcomposite">
				<td width="250">
                                    <?php echo esc_html__('Type', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
                                    
				    <?php echo wp_kses($lists['ftype'], JoomsportSettings::getKsesRadio());?>
                                    
				</td>
			</tr>
                        <tr class="jshideforcomposite jshideforboxtype">
				<td width="250">
                                    <?php echo esc_html__('Fields', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
                                    <?php echo wp_kses($lists['depend1'], JoomsportSettings::getKsesSelect());?>
                                    <?php echo wp_kses($lists['calc'],JoomsportSettings::getKsesSelect());?>
                                    <?php echo wp_kses($lists['depend2'], JoomsportSettings::getKsesSelect());?>
				</td>
			</tr>
                        <?php if($lists['extraf']){?>
			<tr class="jshideforcomposite">
				<td width="250">
                                    <?php echo esc_html__('Connected to', 'joomsport-sports-league-results-management')?>
				</td>
				<td>
                                    <?php ?>
        ...
        <div class="jsBepanel">
            ...
            <div class="jsBEsettings">
                <table>
                    ...
                    <tr>
                        <td width="250">
                                <?php echo esc_html__('Visible on front-end', 'joomsport-sports-league-results-management')?>
                        </td>
                        <td>
                                <?php echo wp_kses($lists['displayonfe'], JoomsportSettings::getKsesRadio());?>
                        </td>
                    </tr>
                    <tr>
                        <td width="250">
    ...
    public static function getListValues($item){
        ...
        $lists['ftype'] = JoomSportHelperSelectBox::Radio('ftype', $is_field,$item['ftype'],'onclick="boxfield_type_hide();"',false);
        ...
        if($efbox){
            $simpleBox = $wpdb->get_results($wpdb->prepare('SELECT id, sel_value as name FROM '.$wpdb->joomsport_ef_select.' WHERE fid=%d ORDER BY eordering,sel_value', $efbox), 'OBJECT') ;
            if(count($simpleBox)){
                ...
                $lists['extraf'] = '<select name="options[extraVals][]" class="jswf-chosen-select" data-placeholder="'.esc_attr(__('Add item','joomsport-sports-league-results-management')).'" multiple>';
                foreach ($simpleBox as $tm) {
                    $selected = '';
                    if(isset($options['extraVals']) && in_array($tm->id, $options['extraVals'])){
                        $selected = ' selected';
                    }
                    $lists['extraf'] .=  '<option value="'.esc_attr($tm->id).'" '.$selected.'>'.$tm->name.'</option>';
                }
                $lists['extraf'] .=  '</select>';
            }
        }
    ...
    public static function joomsport_boxfields_saveselect($item){
        ...
        if($item['complex'] != 1){
            ...
            $is_col = $wpdb->get_results($wpdb->prepare("SHOW COLUMNS FROM {$wpdb->joomsport_box_match} LIKE %s",array($tblCOl)));
